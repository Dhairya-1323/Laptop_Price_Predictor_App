...
cpu=st.selectbox("Processor",df['TypeName'].unique(),index=0)
...
gpu=st.selectbox("Graphics Card on the system",df['Gpu'].unique(),)
...
ssd=st.selectbox("What is the SSD storage on the system? If system has HDD storage only, select 0",[0,128,256,512,1024],index=3)
...
if st.button("PREDICT PRICE"):
  ...
  if touchscreen=='Yes':
    ...
    touchscreen=0
  query=np.array([company,typename,cpu,cpu_speed,ram,gpu,os,weight,ips,touchscreen,ssd,hdd,ppi])
